...
| CM 切片 | [`compute-cm/`](compute-cm/) — `experimental:webgpu-cm@0.4.0`（typed lists/strings + WIT resources + buffer/vertex records + surface/render；**仍非**合规 wasi:webgpu） |
| 下一阶段（已锁定） | **合规 world（无 gfx）**：将 vendor/pin 标准包 `wasi:webgpu@0.3.0-rc.2` 并与 experimental **双轨**并存直至 Guest 迁完；计划 [`docs/scheme/compliant-world.md`](../docs/scheme/compliant-world.md)；缺口 [`docs/mapping/compliant-world-gap.md`](../docs/mapping/compliant-world-gap.md)。**本会话不改** `compute-cm` WIT |
...
合规阶段 vendor 路径（实现切片 A）：预期 `wit/deps/wasi-webgpu/` + `PIN.md`（尚未落仓）。
