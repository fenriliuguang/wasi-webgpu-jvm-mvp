...
| CM 切片（双轨） | [`compute-cm/`](compute-cm/) — `experimental:webgpu-cm@0.5.0`（**仍非**合规）；与标准包并存直至 Guest 迁完 |
...
- **experimental**：现有 Guest（vector-add-cm / triangle-cm）与 `abi-cm` / Linker 仍走 `experimental:webgpu-cm@0.5.0`。  
