...
| CM slice (dual-track) | [`compute-cm/`](compute-cm/) — `experimental:webgpu-cm@0.6.0` (**still not** compliant); coexists with the standard package until Guests migrate |
...
- **experimental**: current Guests (vector-add-cm / triangle-cm) and `abi-cm` / Linker stay on `experimental:webgpu-cm@0.6.0`.  
