...
- **下一阶段（已锁定）：** [工程移交：Maven 可发布化（不对外发布）/ abi-mvp render / 可选 perf](docs/scheme/engineering-handoff.md)（A–C；C 可选不阻塞；**尚未实现**）。仍为 **experimental**；A 仅本地 Publishing 自检，**不做**任何对外发布与「已发布 / 可供依赖」宣称。**不做** wasi-gfx / 合规宣传 / 真 CM async / 上游 PR / 真 dtor overlay。**真机验收基准 = CM cube**
...
| 工程移交（下一阶段，已锁定；不对外发布） | [engineering-handoff](docs/scheme/engineering-handoff.md) | [EN](docs/scheme/engineering-handoff.en.md) |
