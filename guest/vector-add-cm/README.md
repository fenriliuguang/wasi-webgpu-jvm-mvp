...
- **guest-descriptor-cube A/B：** 标准 descriptor（含嵌套 borrow：`create-bind-group` / `pipeline-layout` / `create-compute-pipeline` / `queue.submit(list)`）；需递归 `cm-resources` 补丁 natives
