...
| **Engineering handoff: Maven publishability (no external release) / abi-mvp render / optional perf** | **In progress (A–C)** — plan [`engineering-handoff.en.md`](engineering-handoff.en.md); **A/B done** (A: [`maven-local.en.md`](../maven-local.en.md); B: abi-mvp flat surface/render); C not started. C optional, non-blocking. Remains **experimental**; **no** external release or release claims. **No** wasi-gfx / compliance marketing / true CM async / upstream PRs / true dtor overlay. **Device acceptance baseline = CM cube** (`guest/cube-cm`) |
| **Memo: true CM async / WASI Preview3** | **Memo (not this phase)** — [`true-cm-async-memo.en.md`](true-cm-async-memo.en.md); **revisit only after engineering-handoff lands**. sync-compat remains current |
