...
| **Engineering handoff: Maven publishability (no external release) / abi-mvp render / optional perf** | **Locked (A–C)** — plan [`engineering-handoff.en.md`](engineering-handoff.en.md); **not started**. C optional, non-blocking. Remains **experimental**; A is local Publishing self-check only — **no** external release or release claims. **No** wasi-gfx / compliance marketing / true CM async / upstream PRs / true dtor overlay. **Device acceptance baseline = CM cube** (`guest/cube-cm`) |
