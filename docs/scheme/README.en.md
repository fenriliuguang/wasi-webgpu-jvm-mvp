...
| **Engineering handoff: Maven publishability (no external release) / abi-mvp render / optional perf** | **In progress (A–C)** — plan [`engineering-handoff.en.md`](engineering-handoff.en.md); **A done** ([`maven-local.en.md`](../maven-local.en.md)); B/C not started. C optional, non-blocking. Remains **experimental**; **no** external release or release claims. **No** wasi-gfx / compliance marketing / true CM async / upstream PRs / true dtor overlay. **Device acceptance baseline = CM cube** (`guest/cube-cm`) |
