...
| **CM slice** | **Done (experimental)**: `experimental:webgpu-cm` WIT resources + Guest + `abi-cm` + desktop ComponentLinker → same L2; still not compliant wasi:webgpu |
| **Optional next** | Not started: on-screen / Chicory / Android CM instrumented tests; next can align more wasi:webgpu records/flags |
...
5. Android uses Bionic `libwasmtime4j.so` (`runtime-wasmtime/android-natives`); desktop CM resources need a patched `wasmtime4j-native` (tracked `patches/*.patch` + `scripts/build-wasmtime4j-desktop-cm.ps1`).
