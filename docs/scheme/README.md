...
| **工程移交：Maven / abi-mvp render / 可选 perf** | **已锁定（A–C）** — 计划 [`engineering-handoff.md`](engineering-handoff.md)；**尚未实现**。C 可选不阻塞。**不做** wasi-gfx / 合规宣传 / 真 CM async / 上游 PR / 真 dtor overlay。**真机验收基准 = CM cube**（`guest/cube-cm`） |
