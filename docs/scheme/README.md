...
| **Guest 标准 descriptor 真机 + 旋转纹理立方体** | **进行中** — [`guest-descriptor-cube.md`](guest-descriptor-cube.md)；**A ✅** natives；B 立方体 Demo → C wasi 子集接线 → D 生命周期。Maven / `abi-mvp` render / perf 仍移交 |
