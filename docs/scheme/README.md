...
| **CM 切片** | **完成（experimental）**：`experimental:webgpu-cm` WIT resources + Guest + `abi-cm` + 桌面 ComponentLinker → 同一 L2；仍非合规 wasi:webgpu |
| **后续可选** | 未开始：上屏 / Chicory / Android CM 仪器测试；下一步可对齐更多 wasi:webgpu records/flags |
