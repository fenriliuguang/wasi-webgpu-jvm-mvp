...
| **工程移交：Maven 可发布化（不对外发布）/ abi-mvp render / 可选 perf** | **进行中（A–C）** — 计划 [`engineering-handoff.md`](engineering-handoff.md)；**A 已完成**（[`maven-local.md`](../maven-local.md)）；B/C 尚未实现。C 可选不阻塞。仍为 **experimental**；**不做**对外发布与发布宣称。**不做** wasi-gfx / 合规宣传 / 真 CM async / 上游 PR / 真 dtor overlay。**真机验收基准 = CM cube**（`guest/cube-cm`） |
