...
- [ ] **CI：** [`.github/workflows/ci.yml`](../../.github/workflows/ci.yml) JVM job 增加 `:abi-cm:test`（及低成本 `:abi-wasi:test`）；增加 `publishEngineeredToMavenLocal` 自检（失败即红；不上传远端）
- [ ] **仪器：** [`WasmtimeCmCubeInstrumentedTest`](../../android-demo/src/androidTest/java/io/github/fenriliuguang/wasi/webgpu/demo/WasmtimeCmCubeInstrumentedTest.kt) 在现有 one-shot + 8 帧之外，增加 **同 Session 连续 `runCube` ×N**（默认 3），覆盖历史 D6 二次 CM；测后仍 `releaseAllGpuObjects`
- [ ] **Cpu 不累积：** 巩固 [`AbiCmHostBindingsTest`](../../abi-cm/src/test/) 帧/句柄不累积断言；仪器侧若无计数 API 则保持「无异常完成」
- [ ] **养护检查清单（本页或 dual-runtime-track 短节）：** 假 WIT dtor → `tryDrop` / `releaseLifetimeSafetyNets` / `releaseAllGpuObjects`；进程级 CM linker → 波间 `force-stop` + 单进程复用 Session；门禁命令钉死
- [x] CHANGELOG + 根 README「状态」标明本页为 **现行主线**（立项文档已同步；实现切片仍未开工）

### B — 跟轨 B 扩 Host（正式面收敛）

轨 B 现状仍可能走 deprecated / 快捷路径：`begin-render-pass-clear`、`queueSubmit1`、一步 `surface-get-current-texture-view`。L2 **已有**正式面，本切片负责可依赖化：

- [ ] **正式面标注：** KDoc + [`render-subset.md`](../mapping/render-subset.md) / EN 写明轨 B 应迁到 `commandEncoderBeginRenderPass(descriptor)`、`queueSubmit(list)`、`surfaceGetCurrentTexture` + `textureCreateView`；clear helper / `queueSubmit1` 保留兼容窗口并标 deprecated 迁移目标
- [ ] **单测：** abi-cm / abi-mvp Cpu 路径增加正式 `beginRenderPass` + `queueSubmit(list)` 最小 clear→finish→submit
- [ ] **texture / view 生命周期短契约：** present 后 `tryDrop` / 帧对释放写清，供轨 B 拆提案两步名
- [ ] **本轮不新增** adapter `features` / `limits` / `info`、`deviceDestroy`、`on-submitted-work-done`（等轨 B 明确下一刀再开）
- [ ] 本地 `publishEngineeredToMavenLocal`；CHANGELOG；**不**改轨 B 仓
...
| 真机主门禁 | `./scripts/run-android-instrumented.ps1`（**勿**依赖 Studio UTP） |
